...
<?php $__env->startSection('title', 'កត់ត្រាវត្តមាន | Real-time'); ?>
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl shadow-blue-900/10 border border-white/50 overflow-hidden relative">
                ...
                <div class="relative z-10 px-8 pt-10 pb-8 text-center">
                    <div class="relative inline-block mb-4">
                        <div class="w-24 h-24 rounded-full border-4 border-white shadow-lg bg-gray-100 flex items-center justify-center overflow-hidden">
                            <?php if(Auth::user()->avatar): ?>
                                <img src="<?php echo e(asset('storage/avatars/'.Auth::user()->avatar)); ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <span class="text-3xl font-black text-blue-600"><?php echo e(substr(Auth::user()->name, 0, 1)); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="absolute bottom-1 right-1 w-6 h-6 bg-green-400 border-4 border-white rounded-full"></div>
                    </div>

                    <h2 class="text-2xl font-black text-white drop-shadow-md"><?php echo e(Auth::user()->name); ?></h2>
                    <p class="text-blue-100 text-sm font-medium tracking-wider uppercase mt-1">
                        <?php echo e(\App\Models\Employee::where('user_id', Auth::id())->first()->position ?? 'Staff Member'); ?>
                    ...
                    <div class="mt-8 bg-white/90 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-white/50">
                        ...
                        <p class="text-blue-500 font-bold text-sm mt-1"><?php echo e(date('l, d F Y')); ?></p>
                    </div>

                    <form action="<?php echo e(route('admin.attendances.store')); ?>" method="POST" class="mt-6">
                        <?php echo csrf_field(); ?>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                    ...
                    <div class="hidden md:block px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 shadow-sm">
                        <i class="fas fa-users text-blue-500 mr-2"></i> សរុប: <?php echo e(count($attendances)); ?>
                                ...
                                <tr class="hover:bg-blue-50/30 transition-colors group">
                                    <td class="px-8 py-5">
                                        <div class="flex items-center gap-4">
                                            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-blue-600 font-bold text-sm shadow-sm">
                                                <?php echo e(substr($row->employee->name, 0, 1)); ?>

                                            </div>
                                            <div>
                                                <p class="font-bold text-slate-800 text-sm"><?php echo e($row->employee->name); ?></p>
                                                <p class="text-[10px] text-gray-400 uppercase font-bold"><?php echo e($row->employee->position); ?></p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-50 text-green-700 border border-green-100 text-xs font-bold shadow-sm">
                                            ...
                                            <?php echo e(\Carbon\Carbon::parse($row->check_in)->format('h:i:s A')); ?>
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <?php if($row->check_out): ?>
                                            <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-50 text-gray-600 border border-gray-200 text-xs font-bold">
                                                <i class="fas fa-check"></i>
                                                <?php echo e(\Carbon\Carbon::parse($row->check_out)->format('h:i:s A')); ?>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ASUS ROG\computer-shop\resources\views/admin/attendances/index.blade.php ENDPATH**/ ?>