<nav class="fixed w-full z-[100] top-0 bg-white/90 backdrop-blur-md shadow-sm transition-all duration-300">
    <div class="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="<?php echo e(route('shop.index')); ?>" class="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 flex items-center gap-2">
        ...
        <div class="hidden md:flex space-x-8 font-medium">
            ...
            <?php $__currentLoopData = $navLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(Route::has($link['route']) ? route($link['route']) : '#'); ?>" 
                   class="relative py-1 transition-colors duration-200 <?php echo e(request()->routeIs($link['route']) ? 'text-blue-600 font-bold' : 'text-gray-600 hover:text-blue-600'); ?>">
        ...
        <div class="flex items-center space-x-3 md:space-x-5">
            <a href="<?php echo e(route('cart')); ?>" class="relative p-2 text-gray-600 hover:text-blue-600 transition-transform active:scale-90" title="កន្ត្រកទំនិញ">
                <i class="fas fa-shopping-bag text-xl"></i>
                <?php if(session('cart') && count((array) session('cart')) > 0): ?>
            ...
            <?php if(Route::has('login')): ?>
                <?php if(auth()->guard()->check()): ?>
                    <div class="relative group">
                        <button class="flex items-center space-x-2 text-gray-700 font-semibold hover:text-blue-600 focus:outline-none py-1 text-sm lg:text-base">
                            <div class="relative">
                                <img src="<?php echo e(Auth::user()->avatar ? asset('storage/avatars/'.Auth::user()->avatar) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&background=0D8ABC&color=fff'); ?>" 
                                     alt="Profile" 
                            ...
                            <div class="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden ring-1 ring-black ring-opacity-5">
                                ...
                                <div class="px-5 py-4 border-b border-gray-50 bg-gray-50/50 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm font-bold text-slate-800 truncate w-full px-2"><?php echo e(Auth::user()->email); ?></p>
                                ...
                                <div class="py-2">
                                    <a href="<?php echo e(route('profile.index')); ?>" class="flex items-center gap-4 px-5 py-2.5 text-sm text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                                    ...
                                    <a href="<?php echo e(route('order.history')); ?>" class="flex items-center gap-4 px-5 py-2.5 text-sm text-gray-600 hover:bg-green-50 hover:text-green-600 transition-colors">
                                ...
                                <?php if(Auth::user()->role === 'super_admin' || Auth::user()->role === 'admin'): ?>
                                    ...
                                    <div class="px-5 py-2">
                                        ...
                                        <div class="space-y-1">
                                            <a href="<?php echo e(route('admin.products.index')); ?>" class="flex items-center gap-3 p-2 text-sm text-gray-700 hover:bg-purple-50 rounded-lg transition-colors group/item">
                                                <i class="fas fa-boxes text-purple-500 group-hover/item:scale-110 transition-transform"></i>
                                                <span>គ្រប់គ្រងទំនិញ</span>
                                            </a>
                                            <a href="<?php echo e(route('admin.attendances.index')); ?>" class="flex items-center gap-3 p-2 text-sm text-gray-700 hover:bg-green-50 rounded-lg transition-colors group/item">
                ...
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="bg-slate-900 text-white px-7 py-2.5 rounded-full font-bold hover:bg-blue-600 hover:shadow-lg hover:shadow-blue-200 transition-all active:scale-95 text-sm uppercase tracking-wide">
                        Login
                    </a>
                <?php endif; ?>
            <?php endif; ?>
...
</nav><?php /**PATH C:\Users\ASUS ROG\computer-shop\resources\views/partials/navbar.blade.php ENDPATH**/ ?>