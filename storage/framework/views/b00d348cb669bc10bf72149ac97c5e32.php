...
<?php $__env->startSection('title', 'Customer Profile | ' . $user->name); ?>

<?php $__env->startSection('content'); ?>
<div class="container mx-auto max-w-6xl">
    
    <div class="flex items-center gap-4 mb-10">
        <a href="<?php echo e(route('admin.customers.index')); ?>" class="group w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm hover:shadow-md">
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 text-center relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    ...
                    <h3 class="text-2xl font-black text-slate-900 uppercase tracking-tight italic"><?php echo e($user->name); ?></h3>
                    <p class="text-sm text-slate-400 font-bold mb-8 tracking-tighter"><?php echo e($user->email); ?></p>
                    ...
                    <div class="space-y-4 pt-8 border-t border-slate-50">
                        <div class="flex items-center justify-between text-left">
                            ...
                            <span class="px-3 py-1 bg-slate-900 text-white text-[9px] font-black rounded-lg uppercase tracking-widest"><?php echo e($user->role); ?></span>
                        </div>
                        <div class="flex items-center justify-between text-left">
                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest">Member Since</span>
                            <span class="text-xs font-bold text-slate-600"><?php echo e($user->created_at->format('M d, Y')); ?></span>
                        </div>
                        <div class="flex items-center justify-between text-left">
                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest">Total Orders</span>
                            <span class="text-xs font-black text-blue-600 italic"><?php echo e($orders->count()); ?> Orders</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-[35px] shadow-xl text-white">
                ...
                <div class="flex flex-col gap-3 text-center">
                    ...
                    <a href="mailto:<?php echo e($user->email); ?>" class="mt-2 w-full bg-white/5 hover:bg-white/10 border border-white/10 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">
                ...
                <div class="space-y-8">
                    ...
                    <div class="relative pl-8 before:content-[''] before:absolute before:left-0 before:top-0 before:w-1 before:h-full before:bg-slate-50 before:rounded-full group">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                            <div>
                                <h4 class="font-black italic text-slate-900 tracking-tighter uppercase text-lg">Order #<?php echo e($order->id); ?></h4>
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"><?php echo e($order->created_at->format('d M Y, h:i A')); ?></p>
                            </div>
                            <div class="text-right">
                                <p class="font-black text-blue-600 italic text-2xl tracking-tighter">$<?php echo e(number_format($order->total_amount, 2)); ?></p>
                                <span class="inline-block mt-2 text-[9px] uppercase font-black px-4 py-1.5 rounded-xl tracking-widest shadow-sm border
                                    <?php if($order->status == 'completed'): ?> bg-emerald-50 text-emerald-500 border-emerald-100
                                    <?php elseif($order->status == 'pending'): ?> bg-orange-50 text-orange-500 border-orange-100
                                    <?php else: ?> bg-blue-50 text-blue-500 border-blue-100 <?php endif; ?>">
                            ...
                            <div class="flex items-center gap-4 p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-slate-100 hover:bg-white transition-all duration-300">
                                <div class="w-14 h-14 bg-white rounded-xl flex-shrink-0 overflow-hidden border border-slate-100 shadow-sm">
                                    <?php if($item->product && $item->product->image): ?>
                                        <img src="<?php echo e(asset('storage/' . $item->product->image)); ?>" alt="<?php echo e($item->product->name); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    <?php else: ?>
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-black text-slate-700 uppercase italic tracking-tight truncate"><?php echo e($item->product->name ?? 'Unknown Product'); ?></p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[9px] font-black text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md uppercase tracking-tighter">Qty: <?php echo e($item->quantity); ?></span>
                                        <span class="text-[9px] font-bold text-slate-400">@ $<?php echo e(number_format($item->price, 2)); ?></span>
                                    </div>
                                </div>
                            </div>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ASUS ROG\computer-shop\resources\views/admin/customers/show.blade.php ENDPATH**/ ?>