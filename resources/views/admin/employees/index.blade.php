...
@section('title', 'គ្រប់គ្រងបុគ្គលិក | Admin')
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div class="px-8 py-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-gray-50 to-white gap-4">
            ...
            @if(Auth::user()->role === 'super_admin')
                <a href="{{ route('admin.employees.create') }}" class="bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-blue-600 hover:shadow-lg hover:shadow-blue-200 transition-all flex items-center gap-3 active:scale-95 shadow-md group">
                        ...
                        <tr class="hover:bg-blue-50/30 transition-all group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-bold text-slate-800 text-base tracking-tight leading-none mb-1">{{ $employee->name }}</p>
                                        <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wider">ID: #EMP-{{ str_pad($employee->id, 4, '0', STR_PAD_LEFT) }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-6 text-gray-500 font-bold text-sm tracking-tighter italic">
                                <i class="fas fa-phone-alt mr-2 text-gray-300"></i> {{ $employee->phone ?? 'ពុំមានទិន្នន័យ' }}
                            </td>
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-3">
                                    {{-- ប៊ូតុងកែប្រែ (Edit) --}}
                                    @if(Auth::user()->role === 'super_admin')
                                        <a href="{{ route('admin.employees.edit', $employee->id) }}" class="w-10 h-10 flex items-center justify-center bg-white text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all shadow-sm border border-gray-100 hover:border-blue-200" title="កែប្រែព័ត៌មាន">
                                            <i class="fas fa-edit text-sm"></i>
                                        </a>
                                        
                                        {{-- ប៊ូតុងលុប (Delete) --}}
                                        <form action="{{ route('admin.employees.destroy', $employee->id) }}" method="POST" onsubmit="return confirm('តើអ្នកពិតជាចង់លុបទិន្នន័យបុគ្គលិកនេះចេញពីប្រព័ន្ធមែនទេ?')">
                                            @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-8 py-24 text-center">
                                ...
                                @if(Auth::user()->role === 'super_admin')
                                    <a href="{{ route('admin.employees.create') }}" class="inline-block mt-4 bg-blue-50 text-blue-600 px-6 py-2 rounded-xl font-bold hover:bg-blue-600 hover:text-white transition-all">
